...
public class Add_class extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        batch_edit =  findViewById(R.id.edit_batch);
        subject_edit = findViewById(R.id.edit_subject);
        semester_edit = findViewById(R.id.edit_semester);
        subjectcode_edit = findViewById(R.id.edit_subjectcode);
        stream_edit = findViewById(R.id.edit_stream);
        section_edit = findViewById(R.id.edit_section);
        group_edit = findViewById(R.id.edit_group);

        groupCheck = findViewById(R.id.check_groupcheck);
        ...
        Button bu = findViewById(R.id.realm_browser);
        ...
        imageView=findViewById(R.id.button_import_excel);
        imageView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v)
            {
                if (subject_edit.getText().toString().trim().equalsIgnoreCase(""))
                    ...
                    AlertDialog.Builder alert = new AlertDialog.Builder(com.example.doantotnghiep.Add_class.this);
                    alert.setTitle("Excel Sheet Import");
                    alert.setMessage(" Lưu ý-\n\n" +
                            "- Chỉ sử dụng File Excel XLS, không sử dụng File XLSX.\n\n" +
                            "- File Excel có 5 cột (Từ trái sang phải)-\n \t* Số thứ tự\n \t* Tên\n \t* MSSV\n \t* ID thẻ\n \t* ID thiết bị.\n\n" +
                            "- File Exel nên được tạo bằng Microsoft Excel \n\n");
                    alert.setPositiveButton("Chọn Excel File", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            year = Integer.parseInt(semester_edit.getText().toString().trim());
                            com.example.doantotnghiep.Add_class.this.generateBatchID();
                            ...
                            if(checkBatch.size() == 0) {
                                Intent intent = new Intent(com.example.doantotnghiep.Add_class.this, FilePickerActivity.class);
...
//                                Intent chooserIntent = Intent.createChooser(intent, "Open FILE...");
                                ...
                                launcherSelectFile.launch(intent); //startActivityForResult
                            }
                            else
                                Toast.makeText(com.example.doantotnghiep.Add_class.this,"Môn học này đã được tạo ở học kỳ này", Toast.LENGTH_SHORT).show();
                        }
                    });
...
//        }
//    }
    ActivityResultLauncher<Intent> launcherSelectFile = registerForActivityResult(
...
//                // For JellyBean and above
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                    ...
                    if (clip != null) {
                        for (int i = 0; i < clip.getItemCount(); i++) {
                            Uri uri = clip.getItemAt(i).getUri();
                            Toast.makeText(com.example.doantotnghiep.Add_class.this, "" + uri.toString(), Toast.LENGTH_SHORT).show();
                        }
                    }
                    // For Ice Cream Sandwich
                } else {
                ...
                Excel_sheet_access.readExcelFile(com.example.doantotnghiep.Add_class.this, uri, batchid, subject, subjectcode, batchI, semesterI, stream, section, group);
                Toast.makeText(Add_class.this, "Batch Added!", Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(com.example.doantotnghiep.Add_class.this, Attendance.class);
                startActivity(intent);
                finish();
            }
        }
